...
import { setLanguage, getLanguage } from "./state.js";

$("#lang-select").addEventListener("change", (e) => {
  const lang = e.target.value;
  setLanguage(lang);

  // 번역 변경 시 현재 화면을 언어에 맞게 갱신
...
function bindEvents() {
  ...
  $("#today-btn").addEventListener("click", async () => {
    ...
    if (lang === "kor") {
      ...
      // 영어 → 오늘 추천 구절 지정 (예: John 3:16)
      verse = await fetchOnlineVerse("John 3:16");
    }

    currentResults = [verse];
    currentIndex = 0;
    renderVerse(verse, 0, 1);
    renderInfo(1);
  });
  ...
  const verse = await loadVerseByLanguage(ref);
...
async function refreshCurrentLanguage() {
  ...
  const newVerse = await loadVerseByLanguage(verse.ref);
  if (newVerse) {
    currentResults[currentIndex] = newVerse;
    renderVerse(newVerse, currentIndex, currentResults.length);
  }
}
